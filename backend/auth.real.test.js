// backend/testAPIreal/auth.real.test.js
import { describe, test, expect, beforeAll, afterAll } from '@jest/globals';
import request from 'supertest';
import app from '../src/app.js'; // Đường dẫn này cần ../ để đi ra khỏi testAPIreal
import { db } from '../src/lib/db.js'; // Đường dẫn này cần ../

describe('API Authentication (với DB thật)', () => {
  let server;
  ...
  describe('POST /api/auth/register', () => {
    test('Nên tạo người dùng mới thành công và trả về token', async () => {
      ...
      const dbUser = await db.get('SELECT id, email, name FROM users WHERE email = ?', [newUser.email]);
      expect(dbUser).not.toBeNull();
      expect(dbUser?.name).toBe(newUser.name);

      console.log(`[TEST LOG] Đã xác thực người dùng ${dbUser?.id} tồn tại trong database.`);
    });
    ...
    test('Nên báo lỗi 409 nếu email đã tồn tại', async () => {
      ...
      console.log(`[TEST LOG] Đang thử đăng ký với email đã tồn tại: ${existingUser.email}`);

      const res = await request(server).post('/api/auth/register').send(existingUser);

      expect(res.statusCode).toBe(409);
      expect(res.body.message).toMatch(/Email đã tồn tại|Email already exists/i);

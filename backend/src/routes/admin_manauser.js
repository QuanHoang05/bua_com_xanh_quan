...
async function fetchExtraRoles(userId) {
  ...
  const rows = await dbAll("SELECT role FROM user_roles WHERE user_id = ?", [
    userId,
  ]);
...
async function wipeUserRelated(conn, userId) {
  const exec = async (sql, params = []) => {
    try {
      if (useMySQL && conn.query) {
        await conn.query(sql, params);
      } else {
        await dbRun(sql, params);
      }
    } catch (_) {
      /* bỏ qua lỗi nếu bảng/cột không tồn tại */
    }
  };
  ...
  const plans = [
    ...
    {
      table: "audit_logs",
      sql: "DELETE FROM audit_logs WHERE actor_id = ? OR target_id = ?",
      dual: true,
    },
    {
      table: "notifications",
    ...
    {
      table: "deliveries",
      sql: "DELETE FROM deliveries WHERE shipper_id = ? OR creator_id = ?",
      dual: true,
    },
    {
      table: "delivery_reports",
      sql: "DELETE FROM delivery_reports WHERE reporter_id = ?",
    },
    { table: "bookings", sql: "DELETE FROM bookings WHERE user_id = ?" },
    {
      table: "campaign_followers",
      sql: "DELETE FROM campaign_followers WHERE user_id = ?",
    },
    { table: "comments", sql: "DELETE FROM comments WHERE user_id = ?" },
    {
      table: "messages",
      sql: "DELETE FROM messages WHERE sender_id = ? OR recipient_id = ?",
      dual: true,
    },
  ];
  ...
  try {
    ...
    if (role && role !== "all") {
      if (role === "admin" || role === "user") {
        ...
        if (hasUserRoles) {
          where.push(
            "EXISTS (SELECT 1 FROM user_roles ur WHERE ur.user_id = u.id AND ur.role = ?)"
          );
          params.push(role);
        } else {
    ...
    for (const u of items) {
      ...
      u.extraRoles = u.roles.filter(
        (r) => EXTRA_ROLES.includes(r) && r !== u.role
      );
    ...
    res
      .status(500)
      .json({ ok: false, message: e?.message || "LIST_USERS_ERROR" });
  }
});
...
router.get(
  ...
  async (_req, res) => {
    try {
      ...
      res
        .status(500)
        .json({ ok: false, message: e?.message || "USERS_STATS_ERROR" });
    }
  }
);
...
router.patch(
  ...
  async (req, res) => {
    ...
    try {
      ...
      if (typeof status === "string" && SAFE_STATUS.includes(status)) {
        sets.push("status = ?");
        params.push(status);
      }
      if (sets.length) {
        params.push(id);
        await dbRun(
          `UPDATE users SET ${sets.join(", ")}, updated_at = ${
            useMySQL ? "CURRENT_TIMESTAMP()" : "CURRENT_TIMESTAMP"
          } WHERE id = ?`,
          params
        );
      }
      ...
      if (Array.isArray(roles) && roles.length) {
        const clean = Array.from(
          new Set(roles.map(String).filter((r) => SAFE_ROLES.includes(r)))
        );
        const primary =
          clean.find((r) => r === "admin") ||
          clean.find((r) => r === "user") ||
          clean[0] ||
          "user";
        const extras = clean.filter(
          (r) => EXTRA_ROLES.includes(r) && r !== primary
        );
      ...
      res
        .status(500)
        .json({ ok: false, message: e?.message || "UPDATE_USER_ERROR" });
    }
  }
);
...
router.delete(
  ...
  async (req, res) => {
    ...
    try {
      ...
      return res
        .status(500)
        .json({ ok: false, message: err?.message || "DELETE_USER_ERROR" });
    }
  }
);
...
router.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      res
        .status(500)
        .json({ ok: false, message: e?.message || "RESET_PASSWORD_ERROR" });
    }
  }
);
    ...
    try {
      ...
      try {
        await dbRun(
          ...
          [token, new Date(Date.now() + 3600_000), id]
        );
      } catch {
      ...
      res
        .status(500)
        .json({ ok: false, message: e?.message || "RESEND_VERIFY_ERROR" });
    }
  }
);
...
router.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      res
        .status(500)
        .json({ ok: false, message: e?.message || "IMPERSONATE_ERROR" });
    }
  }
);
...
router.get(
  ...
  async (req, res) => {
    try {
      ...
      if (status && SAFE_STATUS.includes(status)) {
        where.push("u.status = ?");
        params.push(status);
      }
      if (ids) {
      ...
      const csv = [header.join(",")]
        .concat(rows.map((r) => header.map((k) => safeCsv(r[k])).join(",")))
        .join("\n");
      ...
      res
        .status(500)
        .json({ ok: false, message: e?.message || "EXPORT_USERS_ERROR" });
    }
  }
);
function safeCsv(v) {
  ...
  async (req, res) => {
    try {
      ...
      if (!worksheet) {
        return res
          .status(400)
          .json({ ok: false, message: "File Excel không có sheet nào." });
      }
      ...
      for (const r of rows) {
        ...
        try {
          const exist = await dbGet("SELECT id FROM users WHERE email = ?", [
            email,
          ]);
          if (exist) {
            duplicated++;
            ...
            await dbRun(
             ...
             VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
              [
                ...
                email,
                name || email,
                SAFE_ROLES.includes(role) ? role : "user",
                SAFE_STATUS.includes(status) ? status : "active",
                phone,
                address,
                hash,
                nowSql(),
      ...
      res
        .status(500)
        .json({ ok: false, message: e?.message || "IMPORT_USERS_ERROR" });
    }
  }
);
...
router.get("/audit", requireAuth, requireRole("admin"), async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ ok: false, message: e?.message || "AUDIT_LIST_ERROR" });
  }
});

// backend/test/admin.audit.test.js
...
// Giả lập các module DB và middleware để test logic của API một cách độc lập.
...
// Mô tả bộ test cho các route quản lý nhật ký hoạt động (audit log)
describe("Admin Audit Routes (/api/admin/audit)", () => {
  ...
  describe("with SQLite DB", () => {
    ...
    // Test case: Lấy danh sách nhật ký hoạt động thành công (có phân trang)
    test("GET /audit should return a list of audit logs", async () => {
      // Giả lập 2 lần gọi DB: 1 để lấy danh sách items, 1 để lấy tổng số lượng (total)
      sqliteDb.prepare
        .mockReturnValueOnce({ all: () => [{ id: 1, action: 'login' }] })
  ...
  describe("with MySQL DB", () => {
    ...
    // Test case: Lấy chi tiết một nhật ký hoạt động theo ID
    test("GET /audit/:id should return a single audit log", async () => {
      const mockLog = { id: 1, action: 'login' };
      // Giả lập DB trả về một log cụ thể
      mysqlDb.get.mockResolvedValue(mockLog);

      const res = await request(app).get("/api/admin/audit/1").set("Authorization", `Bearer ${adminToken}`);
      
      // Mong đợi status 200 và ID của log trả về là 1
      expect(res.statusCode).toBe(200);

...
const { default: vietqrWebhookRouter } = await import("../src/routes/webhooks.vietqr.js");
...
describe("VietQR Webhook Route (/api/webhooks/bank/vietqr)", () => {
  ...
  test("POST /bank/vietqr should process a valid transaction and update the database", async () => {
    ...
    const res = await request(app).post("/api/webhooks/bank/vietqr").send(webhookData);
